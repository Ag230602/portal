...
const url=import.meta.env.VITE_SUPABASE_URL||'https://yhzrflaunlqmmlgxrthr.supabase.co';
...
export const transport:PortalTransport={
 ...
 async signIn(){if(!supabase)throw new Error('Configure the Supabase publishable key first.');const redirectTo=new URL('./',window.location.href).href;const {error}=await supabase.auth.signInWithOAuth({provider:'google',options:{redirectTo,scopes:'openid email profile'}});if(error)throw error;},
 async signOut(){if(supabase){const {error}=await supabase.auth.signOut();if(error)throw error;}window.location.replace(new URL('./',window.location.href).href);}
};
