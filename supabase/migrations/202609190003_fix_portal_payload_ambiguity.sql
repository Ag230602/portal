-- Disambiguate request arguments from stored record columns.
-- CREATE OR REPLACE preserves function grants and all existing data.
...
begin
 ...
 'reports',(select coalesce(jsonb_agg(r.payload order by r.created_at desc),'[]') from public.naac_reports r where member.role='admin' or r.created_by=uid),
 ...
 if not exists(select 1 from public.naac_departments where name=naac_portal.payload->>'department') or coalesce(trim(naac_portal.payload->>'name'),'')='' then raise exception 'Enter your name and select a valid department.';end if;
 update public.naac_members set name=left(naac_portal.payload->>'name',200),department=naac_portal.payload->>'department',updated_at=t where user_id=uid;
 ...
 if coalesce(trim(naac_portal.payload->>'name'),'')='' or length(naac_portal.payload->>'name')>120 then raise exception 'Enter a department name up to 120 characters.';end if;
 insert into public.naac_departments(name) values(trim(naac_portal.payload->>'name'));
 elsif action='user' then
 if member.role<>'admin' then raise exception 'Administrator access required.' using errcode='42501';end if;
 role_value=naac_portal.payload->>'role';if role_value not in ('admin','coordinator','respondent') or coalesce(naac_portal.payload->>'email','')!~'^[^[:space:]@]+@[^[:space:]@]+\.[^[:space:]@]+$' then raise exception 'Enter a valid email and role.';end if;
 if lower(naac_portal.payload->>'email')=member.email and role_value<>'admin' then raise exception 'You cannot remove your own administrator access.';end if;
 if role_value<>'admin' and not exists(select 1 from public.naac_departments where name=naac_portal.payload->>'department') then raise exception 'Select a valid department.';end if;
 insert into public.naac_members(email,name,role,department) values(lower(naac_portal.payload->>'email'),left(coalesce(nullif(naac_portal.payload->>'name',''),naac_portal.payload->>'email'),200),role_value,case when role_value='admin' then null else naac_portal.payload->>'department' end)
 ...
 raw=naac_portal.payload->'entry';select d.definition into definition from public.naac_panel_definitions d where id=raw->>'panel';
