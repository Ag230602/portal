-- Mechanical eligibility is now a disclaimer, not an approval requirement.
-- Google authentication, draft isolation and sole-owner response/report access remain enforced.
begin;
create function public.naac_can_fill_forms() returns boolean language sql stable security definer set search_path='' as $$
 select exists(select 1 from public.naac_members m join auth.users u on u.id=m.user_id
...
$$;
revoke all on function public.naac_can_fill_forms() from public,anon;
grant execute on function public.naac_can_fill_forms() to authenticated;
create or replace function public.naac_can_read(record_id uuid) returns boolean language sql stable security definer set search_path='' as $$
 ...
 (public.naac_is_owner() or (public.naac_can_fill_forms() and s.created_by=auth.uid() and s.department='Mechanical Engineering' and s.status in ('Draft','Returned'))))
$$;
...
drop function public.naac_is_mechanical_student();
commit;
