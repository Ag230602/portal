-- Preserve the existing sole administrator; only this identity can see responses.
-- Existing records remain intact. Mechanical students require explicit approval.
begin;
alter table public.naac_members add column approved boolean not null default false;
create table public.naac_owner(singleton boolean primary key default true check(singleton),user_id uuid not null unique references auth.users(id));
alter table public.naac_owner enable row level security;
revoke all on public.naac_owner from public,anon,authenticated;
do $$ begin
 if (select count(*) from public.naac_members where role='admin' and user_id is not null)<>1 then
 raise exception 'Exactly one existing signed-in administrator is required to establish portal ownership.';
 end if;
 insert into public.naac_owner(user_id) select user_id from public.naac_members where role='admin' and user_id is not null;
end $$;
create function public.naac_is_owner() returns boolean language sql stable security definer set search_path='' as $$
 select exists(select 1 from public.naac_owner where user_id=auth.uid())
$$;
create function public.naac_is_mechanical_student() returns boolean language sql stable security definer set search_path='' as $$
 select exists(select 1 from public.naac_members where user_id=auth.uid() and approved and department='Mechanical Engineering' and role='respondent')
$$;
create or replace function public.naac_role() returns text language sql stable security definer set search_path='' as $$
 select case when public.naac_is_owner() then 'admin' else 'respondent' end
$$;
create or replace function public.naac_can_read(record_id uuid) returns boolean language sql stable security definer set search_path='' as $$
 select exists(select 1 from public.naac_submissions s where s.id=record_id and
 (public.naac_is_owner() or (public.naac_is_mechanical_student() and s.created_by=auth.uid() and s.department='Mechanical Engineering' and s.status in ('Draft','Returned'))))
$$;
create or replace function public.naac_can_upload(record_id uuid) returns boolean language sql stable security definer set search_path='' as $$
 select exists(select 1 from public.naac_submissions s where s.id=record_id and s.status in ('Draft','Returned') and public.naac_can_read(s.id))
$$;
alter policy naac_revisions_read on public.naac_revisions using(public.naac_is_owner());
alter policy naac_reviews_read on public.naac_reviews using(public.naac_is_owner());
alter policy naac_reports_read on public.naac_reports using(public.naac_is_owner());
alter policy naac_audit_read on public.naac_audit using(public.naac_is_owner());
alter policy naac_members_read on public.naac_members using(user_id=auth.uid() or public.naac_is_owner());
revoke all on function public.naac_is_owner(),public.naac_is_mechanical_student() from public,anon;
grant execute on function public.naac_is_owner(),public.naac_is_mechanical_student() to authenticated;
...
declare
 ...
 data_year text;data_department text;data_panel text;role_value text;object_meta jsonb;expected_version integer;missing text[]; student_email text; approved_count integer=0;
begin
 ...
 if action='load' then
 return jsonb_build_object('user',jsonb_build_object('email',member.email,'name',member.name,'role',member.role,'department',case when member.role='admin' then 'Mechanical Engineering' else coalesce(member.department,'') end,'approved',public.naac_is_owner() or public.naac_is_mechanical_student()),
 ...
 'users',(select coalesce(jsonb_agg(jsonb_build_object('email',email,'name',name,'role',role,'department',department,'approved',approved,'owner',user_id=(select user_id from public.naac_owner))),'[]') from public.naac_members where member.role='admin'),
 ...
 if not public.naac_is_mechanical_student() then raise exception 'Your Gmail account must be approved by the administrator as a Mechanical Engineering student.' using errcode='42501';end if;
 ...
 if action in ('approveStudents','user') then
 if action='user' then raw=jsonb_build_array(naac_portal.payload->>'email');else raw=naac_portal.payload->'emails';end if;
 if jsonb_typeof(raw) is distinct from 'array' or jsonb_array_length(raw)=0 then raise exception 'Enter at least one student email address.';end if;
 -- No enrollment cap. A batch can contain more than 100 students.
 for student_email in select distinct lower(trim(v)) from jsonb_array_elements_text(raw) v loop
 if student_email is null or length(student_email)>254 or student_email!~'^[^[:space:]@]+@[^[:space:]@]+\.[^[:space:]@]+$' then raise exception 'Invalid student email: %',coalesce(student_email,'(empty)');end if;
 if student_email=member.email then raise exception 'Your owner account already has full access; exclude it from the student list.';end if;
 insert into public.naac_members(email,name,role,department,approved) values(student_email,student_email,'respondent','Mechanical Engineering',true)
 on conflict(email) do update set role='respondent',department='Mechanical Engineering',approved=true,updated_at=t;
 approved_count=approved_count+1;
 end loop;
 result=jsonb_build_object('ok',true,'approved',approved_count);
 elsif action='revokeStudent' then
 student_email=lower(trim(naac_portal.payload->>'email'));
 if student_email=member.email then raise exception 'Owner access cannot be revoked.';end if;
 update public.naac_members set approved=false,updated_at=t where email=student_email;
 elsif action in ('department','onboard') then
 raise exception 'This portal accepts only administrator-approved Mechanical Engineering students.' using errcode='42501';
